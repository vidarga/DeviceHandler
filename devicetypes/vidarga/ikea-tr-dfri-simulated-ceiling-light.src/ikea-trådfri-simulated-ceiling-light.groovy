 ...
 *  Author: SmartThings
 *  Date: 2017-08-07
 *  Adjusted for IKEA-Trådfri bulbs. Temperatur adjustment and control with WebCore
 */
...
@Field final Integer  COLOR_TEMP_DEFAULT = 2700 //COLOR_TEMP_RANGE.getFrom() + ((COLOR_TEMP_RANGE.getTo() - COLOR_TEMP_RANGE.getFrom())/2)
...
metadata {
    definition (name: "ikea-trådfri-simulated-ceiling-light", namespace: "vidarga", author: "vidarga") {
        ...
        command	   "setLowColorTemp"
        command	   "setMidColorTemp"
        command	   "setHighColorTemp"
    }
    ...
    tiles(scale: 2) {
        ...
        controlTile("colorTempControlSlider", "device.colorTemperature", "slider", width: 4, height: 1, inactiveLabel: false, range: "(2200..4000)") {
            state "colorTemperature", action: "setColorTemperature"
        }

        standardTile("refresh", "device.switch", inactiveLabel: false, decoration: "flat", width: 3, height: 1) {
            state "default", label: "", action: "", icon: "st.secondary.refresh"
        }
        ...
        standardTile("LowColorTemp", "device.switch", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "default", label: "Warm", action: "setLowColorTemp"
        }
        
        standardTile("MidColorTemp", "device.switch", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "default", label: "Mid", action: "setMidColorTemp"
        }        

        standardTile("HighColorTemp", "device.switch", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "default", label: "Cold", action: "setHighColorTemp"
        }

        main(["switch"])
        details(["switch", "colorTempControlLabel", "colorTempControlSlider", "bulbValue", "colorIndicator", "refresh", "reset", "LowColorTemp", "MidColorTemp", "HighColorTemp"])
    }
}
...
private setLowColorTemp() {
    log.trace "Executing setLowColorTemp"
    setColorTemperature(2200)
    done()
}
private setMidColorTemp() {
    log.trace "Executing setMidColorTemp"
    setColorTemperature(2700)
    done()
}

private setHighColorTemp() {
    log.trace "Executing setHighColorTemp"
